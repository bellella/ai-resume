...
export const updatePersonalInfo = async (data: PersonalInfo): Promise<ApiResponse<PersonalInfo>> => {
  return await api.patch('users/personal-info', {
    json: data,
  }).json();
};
 ...
 */
export const updateDefaultResume = async (defaultResumeJson: ResumeJson): Promise<ApiResponse<ResumeJson>> => {
  return await api.patch('users/me/resume', {
...
export const updateUserImage = async (formData: FormData): Promise<ApiResponse<string>> => {
  return await api.patch('users/me/image', {
    body: formData,
  }).json();
};
